its my reuseable component:
<template>
  <div class="flex flex-col items-center justify-center gap-y-5 px-28 py-14">
    <!-- Upload -->
    <p v-if="props.fileSize || props.checkFormat" class="text-center text-red-600">{{ props.fileSize === true ? 'File size exceeded the limit of 25 MB' : '' || props.checkFormat === true ? 'Wrong format file. Please upload only mp4 files.' : '' }}</p>
    <div class="relative mx-auto flex w-48 cursor-pointer items-center justify-center overflow-hidden rounded-lg bg-[#b53836ff] py-4 text-xl text-white duration-300 hover:font-semibold hover:shadow-2xl">
      <input type="file" name="uploadFile" :accept="props.selectedFormat !== '...' ? props.selectedFormat : ''" class="absolute bottom-0 left-0 right-0 top-0 m-0 cursor-pointer p-0 opacity-0 filter-none" @change="checkFileSize" required />
      <svg :class="props.fileSize === null ? 'hidden' : 'block'" xmlns="http://www.w3.org/2000/svg" class="mr-2 h-7 w-7" fill="currentColor" viewBox="0 0 104.69 122.88" style="enable-background: new 0 0 104.69 122.88" xml:space="preserve">
      ...
      <svg :class="props.fileSize === null ? 'block' : 'hidden'" xmlns="http://www.w3.org/2000/svg" class="mr-2 h-7 w-7" fill="currentColor" viewBox="0 0 107.07 122.88" style="enable-background: new 0 0 107.07 122.88" xml:space="preserve">
        <g>
    ...
    <div v-if="props.fileSize !== null" class="shadow-xl">
      <video v-if="previewUrl" width="350" height="200" class="mt-5 rounded-md">
        <source :src="previewUrl" type="video/mp4" />
      </video>
    </div>
    <!-- loading -->
    <div class="mt-1 text-center" :class="props.loadingBar === '' ? 'hidden' : 'block'">
...
const checkFileSize = (event) => {
  if (event.target.files && event.target.files.length > 0) {
    ...
    // extension
    props.selectedFormat = file.name.slice(file.name.lastIndexOf('.'));
    ...
    if (file.size > 50000000) {
      ...
      props.checkFormat = false;
      props.fileSize = false;
