  ...
  <div class="grid h-[350px] bg-[#363636ff] px-28 pt-20 text-white lg:grid-cols-2">
    ...
    <div class="flex justify-center text-center maxlg:pb-12">
      <div class="mt-10 font-semibold">
        ...
        <select v-model="GlobalData.selectedImageFileFormat" name="ConvertFromSelect" selected=".mp4" class="w-36 rounded-lg border bg-[#363636ff] px-4 py-3 text-lg outline-none">
          <option v-for="(option, index) in imageFormats" :key="index" :value="option.value">{{ option.label }}</option>
        </select>
        <span class="mx-3">to</span>
        <select v-model="GlobalData.imageSelectedFormat" name="selectMenu" selected=".mp4" class="w-36 rounded-lg border bg-[#363636ff] px-4 py-3 text-lg outline-none">
